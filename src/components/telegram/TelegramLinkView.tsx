    ...
    <div className="min-h-screen bg-stone-100 flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-sm p-6">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-stone-900">{window.location.href.includes('admin') || (window as any).Telegram?.WebApp?.initDataUnsafe?.start_param?.includes('admin') ? (am ? 'የህንፃ አስተዳደር' : 'Building Management') : 'BuildingOS'}</h1>
          <p className="text-stone-500 mt-1">{window.location.href.includes('admin') || (window as any).Telegram?.WebApp?.initDataUnsafe?.start_param?.includes('admin') ? (am ? 'የአስተዳዳሪ መግቢያ' : 'Admin Portal') : (am ? 'የተከራይ መግቢያ' : 'Tenant Portal')}</p>
        </div>
